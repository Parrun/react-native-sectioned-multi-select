...
class SectionedMultiSelect extends PureComponent {
  static propTypes = {
    isSelectedItemOnLeft: PropTypes.bool,
    isShowTagUnderSearchModal: PropTypes.bool,
  ...
  _getSelectLabelWithTag = () => {
    ...
    return single || selectedItems.length >= 1  ? 
      this._displaySelectedItemsWithTag(selectLabelNumberOfLines):
      this._renderSelectTexrComponent(selectLabelNumberOfLines, label)
  }

  _renderSelectTexrComponent = (
  ...
  _renderChips = () => {
    ...
    return selectedItems.length > 0 && !single && showChips && !customChipsRenderer ? (
      ...
      >
        ...
        {
          !isShowTagInSearchTextInput && this._displaySelectedItems()
        }
      </View>
    ) : null
  }
  ...
  _displaySelectedItemsWithTag = (selectLabelNumberOfLines?: mumber) => {
    const {
      uniqueKey,
      subKey,
      selectedItems,
  ...
  _renderFooter = (selectedItemLength: number) => {
    const { footerComponent } = this.props
    const paddingTopNumber = selectedItemLength > 0 ? (selectedItemLength + 1) * 120 : 0;
    return <View style={{ paddingTop: paddingTopNumber }}>{footerComponent && callIfFunction(footerComponent)}</View>
  }

  _renderItemFlatList = ({ item }) => {
    const { styles, colors, searchTerm } = this.state
    const { isSelectedItemOnLeft } = this.props;
    return (
      <View>
        <RowItem
          iconRenderer={Icon}
          ...
          isSelectedItemOnLeft={isSelectedItemOnLeft}
  ...
  render() {
    ...
    const isItemSelected = single || selectedItemLength >= 1
    const labelTagContainer = getLabelTagContainerStyle(isItemSelected)
    ...
    return (
      <View>
                ...
                {loading ? (
                  ...
                  <View>
                    {!renderItems || (!renderItems.length && !searchTerm)
                      ...
                      : null}
                    {
                      (isItemSelected && isShowTagUnderSearchModal) && (
                      <View style={{
                        borderColor: '#EFEFF4',
                        borderWidth: 1,
                        borderStyle: 'dashed',
                    ...
                    {items && renderItems && renderItems.length ? (
                      <View style={{ 
                        ...
                        <FlatList
                          ...
                          ListFooterComponent={this._renderFooter(selectedItemLength)}
        ...
        {!hideSelect && !isShowTagInSearchTextInput && (
          <TouchableWithoutFeedback
              ...
              style={[
                {
                  ...
                  paddingVertical: isItemSelected ? 4 : 12,
                  borderWidth: 1,
                  borderColor: '#cbcacf',
                  borderRadius: 8,
                  padding: 14,
                ...
                <View style={labelTagContainer}>
                    ...
                    <View
                      style={[
                        {
                          flex: 0.15,
                          minHeight: 24 * (selectedItemLength + 1),
                          flexDirection: 'row',
...
const getLabelTagContainerStyle = (isSelected: boolean) => {
  let style = {};
  if (isSelected) {
    style = {
      flex: 0.85,
